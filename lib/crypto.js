...
exports.hashObject = function (obj) {
  ...
  Object.keys(obj).forEach(function (k) {
    if (!k.match(/^(hash|signature)$/)) copy[k] = obj[k];
  });
...
exports.sign = function (keyring, stat, cb) {
  if (!stat.hash) return cb(new Error('stat has no hash for signing'));
  exports.initPrivate(keyring, function (err) {
    if (err) return cb(err);
    var ret = keyring.privateKey.sign('sha256', stat.hash, 'base64', 'buffer');
...
exports.verify = function (keyring, stat, cb) {
  ...
  function withPublicKey (publicKey) {
    if (!publicKey.verify('sha256', stat.hash, stat.signature, 'base64')) {
