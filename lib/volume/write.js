        ...
        pipeline.push(es.through(function write (data, encoding) {
          ...
          function withSignature () {
            if (stat.cipher) {
              ...
              // use that. otherwise use opts.password. 'rsa' cipher uses
              // volume default cipher.
              var cipher;
              if (key) cipher = crypto.createCipher(stat.cipher, key ? key.password : opts.password);
              else cipher = crypto.createCipher(stat.cipher === 'rsa' ? volume.cipher : stat.cipher, opts.password);
