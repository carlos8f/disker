...
module.exports = function () {
  ...
  function initPrivate (cb) {
    ...
    if (!keyring.privateKey) {
      if (keyring.passphrase) {
        prompt.password('Enter password: ', withPassphrase);
      }
      else withPassphrase(undefined);

      function withPassphrase (passphrase) {
        fs.readFile(path.join(keyring.path(), 'key'), {encoding: 'ascii'}, function (err, pem) {
          if (err) return cb(err);
          debug('pem', pem);
          keyring.privateKey = ursa.createPrivateKey(pem, passphrase);
          cb();
        });
      }
    }
