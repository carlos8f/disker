...
module.exports = exports = {
  ...
  load: function (p, opts, cb, keyring) {
    ...
    var volPath = p
      ...
      , lockPath = path.join(volPath, 'lock')
      , lock
    ...
    meta.read(metaPath, function (err, volume) {
      ...
      // lock the volume for writing
      volume.lock = function (cb, tries) {
        if (!tries) tries = 0;
        fs.exists(lockPath, function (exists) {
          if (exists) {
            if (++tries < 5) {
              return setTimeout(function () {
                volume.lock(cb, tries);
              }, 30);
            }
            var err = new Error('volume is currently locked');
            err.code = 'E_LOCKED';
            return cb(err);
          }
          lock = idgen();
          fs.writeFile(lockPath, lock, cb);
        });
      };

      // release lock
      volume.unlock = function (cb) {
        fs.readFile(lockPath, {encoding: 'utf8'}, function (err, _lock) {
          if ((err && err.code === 'ENOENT') || _lock !== lock) {
            err = new Error('lock was broken, cannot commit');
            err.code = 'E_UNLOCKED';
            return cb(err);
          }
          if (err) return cb(err);
            ...
            function normalPipeline () {
              ...
              pipeline.push(es.through(function write (data, encoding) {
                ...
                meta.write(volume.path(p), stat, function (err) {
                  ...
                  volume.journal('+', stat, function (err) {
      ...
      // sync volume state and other stats, and release lock.
      volume.journal = function (op, stat, cb) {
        meta.read(metaPath, function (err, vol) {
          if (err) {
            volume.unlock(function () {
              cb(err);
            });
          ...
          vol.clock++;
          vol.state = crypto.hash(Buffer.concat([
            Buffer(vol.state, 'base64'),
            Buffer(op),
            Buffer(stat.hash, 'base64')
          ]));
          vol.updated = new Date().getTime();
          meta.write(metaPath, vol, function (err) {
            if (err) {
              volume.unlock(function () {
                cb(err);
              });
            ...
            volume.clock = vol.clock;
            volume.state = vol.state;
            volume.updated = vol.updated;
            volume.unlock(cb);
          });
        });
      };
